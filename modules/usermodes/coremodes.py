...
def umode_S_isok(client):
	if Usermode.allow_services(client):
		return 1
	if client.has_permission("self:become-service"):
		return 1
	return 0
...
def umode_u_isok(client):
	if Usermode.allow_none(client):
		return 1
	if client.has_permission("self:become-ulined"):
		return 1
	return 0
...
def umode_q_isok(client):
	if Usermode.allow_services(client):
		return 1
	if client.has_permission("self:become-protected"):
		return 1
	return 0
...
def umode_p_isok(client):
	if Usermode.allow_services(client):
		return 1
	if client.has_permission("self:become-immutable"):
		return 1
	return 0
...
def init(module):
	...
	Usermode.add(module, 'q', 1, 1, umode_q_isok, "Protected on all channels [IRCops only]")
